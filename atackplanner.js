if(!$("#planer_klinow").length){
	...
	var dane = {
		...
		nazwyWojsk:["M˝zrakl˝","K˝l˝Ál˝","Baltac˝","Casus","Hafif Atl˝","A˝r Atl˝","KoÁba˛˝","Manco","ﬁovalye","Misyoner"]
	};
...
function wypiszMozliwosci(){
	if(pobieram){$("#ladowanie").html("Czekaj, muszÍ pobraÊ..."); setTimeout(wypiszMozliwosci, 500); return;}
	...
	for(i=0;i<mojeWioski.length;i++){
		...
		if(najwolniejsza != 0){
			...
			html[ilosc_wiosek]=htmlTmp[i]+"<td>"+ddd+"<td>"+0+"<td><a href='"+dane.linkDorozkazu+id[i]+"&screen=place&x="+cel[0]+"&y="+cel[1]+mozliwewojska+"'>Gˆnder</a>";
		tabelkaBB[ilosc_wiosek]="[*]"+dane.nazwyWojsk[najwJednostka]+"[|] "+mojeWioski[i][mojeWioski[i].length-3]+"|"+mojeWioski[i][mojeWioski[i].length-2]+" [|] "+cel[0]+"|"+cel[1]+" [|] "+ddd+" [|] [url=https://"+document.URL.split("/")[2]+dane.linkDorozkazu+id[i]+"&screen=place&x="+cel[0]+"&y="+cel[1]+mozliwewojska+"]Gˆnder\n";
			ilosc_wiosek++;
		}
		else{
			htmlTmp[i]  = "";
	...
	if(ilosc_wiosek==0) UI.InfoMessage('Nie zmieszczÍ øadnego rozkazu w podany termin :( ', 1500, 'error');
	...
	$('#lista_wojska tbody').html(html.join("\n")+(ilosc_wiosek?"<tr><td id='export_bb' colspan="+(dane.predkosci.length+4)+"><a href='#' onclick=\"$('#export_bb').html('<textarea cols=100 rows=2 onclick=\\'this.select()\\'>[table][**]Birim[||]èKaynak[||]Hedef[||]Kalk˝˛ Zaman˝[||]Emir[/**]\\n'+tabelkaBB.join('')+'[/table]</textarea>');\" ><img src='"+image_base+"igm/export.png' > Listeyi D˝˛a Aktar</a>":''));
...
function wybieranieWiosek(){
	...
	okienko = "<tr><th style=\"cursor:pointer;\" onclick=\"ustaw_min(0); $('#wyborWojsk tr:has(td)').each(function(i){$(this).show();}); \">Asgari ASker Say˝s˝:";
	for(i=0;i<dane.predkosci.length;i++)
	...
	okienko +="<th style=\"cursor:pointer;\" onclick=\"sortowanie_przegladu("+(obrazki.length)+");\" >Mesafe<th><input type='checkbox' onClick='zaznaczWszystko(this)'\" >";
...
function rysujPlaner(){
	...
	elem += "<th>Kalk˝˛ Zaman˝<th><span class=\'icon header time\'><th><b>Emir</b></thead>";
...
function pobierzDane(){
	...
	function processResponse(){
		if (r.readyState == 4 && r.status == 200) {
			...
			if(!tabela){ $("#ladowanie").html("W\u00A0wybranej\u00A0grupie nie\u00A0ma\u00A0wiosek\u00A0:/ Wybierz\u00A0innπ"); pobieram = false; return;}
